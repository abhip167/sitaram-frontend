...
export default {
  ...
  methods: {
    getPdf() {
      var self = this;
      self.pdfdata = pdfvuer.createLoadingTask("./file.pdf");
      self.pdfdata.then((pdf) => {
        ...
        var sticky = self("#buttons")[0].offsetTop;

        // Add the sticky class to the self.$refs.nav when you reach its scroll position. Remove "sticky" when you leave the scroll position
        function stickyNav() {
          if (window.pageYOffset >= sticky) {
            self("#buttons")[0].classList.remove("hidden");
          } else {
            self("#buttons")[0].classList.add("hidden");
          }
        }
